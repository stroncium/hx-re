...
abstract MatchOfIter(MatchIterator) from MatchIterator{
  public var pos(get, never):Int;
  public var len(get, never):Int;

  inline function get_pos() return -1;
  inline function get_len() return -1;
...
abstract MultiMatch(js.RegExp.RegExpMatch) from js.RegExp.RegExpMatch{
  public var pos(get, never):Int;
  public var len(get, never):Int;

  inline function get_pos() return this.index;
  inline function get_len() return this[0].length;
  public inline function group(n:Int) return this[n];
}

@:access(RE.re)
class MatchIterator{
  var off:Int;
  var nextMatch:js.RegExp.RegExpMatch;
  var re:js.RegExp;
  var str:String;
  public function new(re:RE, str:String, ?off:Int, ?len:Int){
    if(off == null) off = 0;
    if(len != null) str = str.substr(0, len);
    this.off = off;
    this.str = str;
    this.re = re.re;
    this.re.lastIndex = off;
    nextMatch = this.re.exec(str);
  }

  public inline function iterator() return this;

  public inline function hasNext() return nextMatch != null;

  public inline function next(){
    var ret = (nextMatch:MultiMatch);
    var re = re;
    re.lastIndex = (off = off + ret.len);
    nextMatch = re.exec(str);
    return ret;
  }
...
class RE{
  ...
  public function new(txt:String, ?opts:String){
    this.txt = txt;
    this.opts = opts;
    #if js if(opts != null && opts.indexOf('s') != -1) throw '"s" option not implemented'; #end

    #if flash9
      re = untyped __new__(__global__["RegExp"], txt, opts == null ? '' : opts);
    #elseif (neko || cpp)
      re = regexp_new_options(txt, opts == null ? '' : opts);
    #elseif php
      re = "/" + untyped __php__("str_replace")("/", "\\/", txt) + "/" + (opts == null ? '' : opts);
    #elseif js
      re = new js.RegExp(txt, opts == null ? '' : opts);
    #end
  }

  public function matches(str:String, ?off:Int, ?len:Int):Bool{
    //TODO len
    #if js
      if(len == null){
        if(off != null) re.lastIndex = off;
      }
      else{
        str = str.substr(off == null ? 0 : off, len);
      }
    ...
    #elseif (neko || cpp)
      if(off == null) off = 0;
      return regexp_match(re, str, off, str.length - off);
    #elseif php
      if(off == null) off = 0;
      return (untyped __php__("preg_match")(re, str, null, __php__("PREG_OFFSET_CAPTURE"), off) > 0);
    #end
  }

  // public function match(str:String, ?off:Int, ?len:Int):Null<SingleMatch>{
  //   #if js
  //     if(len == null){
  //       if(off != null) re.lastIndex = off;
  //     }
  ...
  //     return re.exec(str) != null;
  //   #elseif flash9
  //     if(off != null) r.lastIndex = off;
  //     return re.exec(s) != null;
  //   #elseif (neko || cpp)
  //     if(offset < 0){
  ...
  public inline function matchAll(str:String, ?off:Int, ?len:Int) return new MatchIterator(this, str, off, len);
  ...
  public inline function toString() return opts == null ? '~/$txt/' : '~/$txt/$opts';
}
